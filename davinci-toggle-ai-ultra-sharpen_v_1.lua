-- DaVinci Resolve Toggle AI Ultra Sharpen Nodes v_1 (Lua)
--
-- Scans every timeline in the current project for color nodes that appear to
-- contain Resolve AI Ultra Sharpen tools, then alternates disabling/enabling
-- those whole nodes.
--
...
local LOG_FILENAME = "resolve_toggle_ai_ultra_sharpen_v_1_log.txt"
local STATE_FILENAME = "resolve_toggle_ai_ultra_sharpen_v_1_state.txt"
...
local function tool_matches_ai_ultra_sharpen(tool_name)
...
local function node_has_ai_ultra_sharpen(graph, node_index)
    ...
    for _, tool_name in pairs(tools) do
        ...
        if tool_matches_ai_ultra_sharpen(tool_name) then
            matched = true
        end
    end
...
local function process_item(item, timeline_index, timeline_name, track_index, item_index, target_enabled)
    ...
    for node_index = 1, node_count do
        local matched, tool_summary = node_has_ai_ultra_sharpen(graph, node_index)
        if matched then
            matches = matches + 1
...
local function main()
    log_line("Resolve Toggle AI Ultra Sharpen Nodes v_1 (Lua)")
    log_line("----------------------------------------------")
